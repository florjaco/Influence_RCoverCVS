function [PRV, Vrv, Qtc, Qpv, PLV, Vlv, Qmt, Qav, Vtot, T, Vvc, Vpa, Vpu, Vao, PVC, PPA, PPU,PAO]=closed_loop2(ploteo)
    global HR Rpul Ees_vc Ees_pa Ees_pu Ees_ao Rsys Vi_rv Vi_lv Vi_pu Vi_ao Vi_vc Vi_pa Qi_tc Qi_mt Qi_pul Qi_sys V0_spt V0_rvf V0_lvf V0_pcd Vd_spt Vd_lvf Vd_rvf lambda_spt lambda_lvf lambda_rvf lambda_pcd Ees_spt Ees_rvf Ees_lvf P0_spt P0_lvf P0_rvf P0_pcd Pth Rtc Ltc Lpv Rpv Rmt Lmt Rav Lav Pra Ppa Ppu Pla Pao Pvc
...
%     Vi_spt=2.4*10^(-6);                         %10^(-6)[m^3]=6.5mL
 ...
 % arteria pulmonar, vena pulmonar y aorta (se agregan esas 4 camaras al cerrar el loop)
    ...
    Pth=-4.59*133.322;%-3.68*133.322;%-5.88*133.322;%-4*133.322;                              %-4mmHg pasado a Pa; esto deberia ser oscilatorio para simular inspiracion y espiracion; presion promedio del torax. en ventilacion respiratoria mecanica Pth esta aumentado: Pth=0 mmHg
...
PPA(k,1)=Ees_pa*Vpa(k)+Pth;
PPU(k,1)=Ees_pu*Vpu(k)+Pth;
...
figure();
plot(T,Vrv*10^6,T,Vlv*10^6),xlabel('tiempo [s]'),ylabel('volumenes ventriculares [mL]'), legend('VD','VI'),grid on;
figure();
...
%                 pepe=Funcionpepe(Vspt,t,VRV*10^6,VLV*10^6);
                ...
                Pperi= Ppcd + Pth; %Pth= presion en el torax
